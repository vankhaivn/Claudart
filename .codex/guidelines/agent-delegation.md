...
| ---------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `routine`  | focused search, call-site inventory, docs lookup, extraction, mechanical or highly constrained edit                       | `gpt-6-luna` / `high`  |
| `standard` | bounded implementation or debugging with clear contracts and local verification                                           | `gpt-6-sol` / `medium` |
| `complex`  | ambiguous cross-file debugging, architecture-sensitive reasoning, difficult review or integration                         | `gpt-6-sol` / `max`    |
...
Luna/high is the preferred profile for narrow work that still needs synthesis or completeness checks. Use Luna/low for genuinely mechanical lookup, extraction, or transformation with an easy correctness check, not merely because a task is short. Sol/medium is the default implementation profile; Sol/max is the deliberate quality-oriented profile for `complex` work. Reserve Astra/high for justified `maximum` work, allowing lower effort when sufficient. A failed lower-effort attempt is not required.

The **parent session's selected model and reasoning effort are hard ceilings for implicit delegation**. Treat the child execution profile as two independent dimensions: select the suitable class profile, including the lower-effort exceptions above, then cap each dimension at the parent session's selected setting. A difficult task is not permission to spend above the user's session choice on either dimension.
...
- After capping either dimension or selecting a supported effort, reassess whether the effective profile can still complete the unit reliably. Narrow the unit, work locally, or choose another known suitable profile within the ceilings; a valid cap alone does not establish suitability.
- A ceiling is not a default: do not inherit the parent's xhigh/max/ultra merely because Astra is needed. Higher Astra effort needs a concrete benefit for that unit within the authorized ceilings, not just a stronger parent setting.
- An Astra/max parent may route a `routine` unit to Luna/high, a `standard` unit to Sol/medium, a `complex` unit to Sol/max, and reserve Astra/high for `maximum` work.
- An Astra/high parent caps `complex` work at Sol/high, not Sol/max. A Sol/medium parent caps both `complex` and `maximum` units at Sol/medium. An Astra/low parent cannot implicitly launch Luna/high even though Luna is a cheaper model. Reassess the capped profile's suitability in each case.
...
- If the ideal effort is unsupported by the selected child model, choose the nearest supported effort at or below both the intended effort and the parent-effort ceiling, then reassess suitability. Never compensate for an unsupported effort by silently raising the model or effort above either ceiling.
- Use the exact model slug exposed by the current `spawn_agent` model catalog. The intended routing order is `gpt-6-luna` → `gpt-6-sol` → `gpt-6-astra`. These are project routing/authorization tiers, not a universal benchmark ranking or a guarantee of total task cost. Availability can change, so respect account/workspace allowlists and never substitute a model above the parent ceiling.
- If the parent model/effort cannot be identified, the desired child profile is unavailable or disallowed, or the override cannot be trusted, do not guess upward: work locally or use a known suitable available profile within the ceilings. Do not silently fall back to an older model generation; make material profile substitutions explicit. Inheritance still requires the suitability check above.
