---
paths: ["**/*"]
description: Mission-scale spec workspaces in `.codex/specs/` — a POC-frozen SPEC plus a decision-complete ROADMAP that any later session (often a cheaper model) executes autonomously until done, with self-QA, circuit breakers, and session rotation.
...
4. User confirms → `done`: update INDEX, append one line to `.codex/JOURNAL.md` (`YYYY-MM-DD | completed | spec <slug> — <one-line outcome>`), and sweep `NOTES.md` before shelving — graduate remaining `→ graduate: knowledge/` facts (register in its INDEX), propose `$codex-learn` for flagged behavioral lessons. The folder stays as archive; its durable cargo leaves first. User reports a problem → back to `running`: un-tick the affected tasks/scenarios, log the report verbatim in LEDGER, resume the loop.
...
- **CONTEXT.md**: may carry one pointer line (`Running spec \`<slug>\` (see .codex/specs/<slug>/SPEC.md)`); never absorbs spec content.
