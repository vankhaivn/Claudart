...
Trim `AGENTS.md` so it stays a root loader, not a knowledge dump. In this template, `AGENTS.md` is sourced from `.codex/AGENTS.md`. It should contain only:

- Project identity
- Context loading order
- Core Codex workflows
- Links to `.codex/CODEX.md`, `.codex/CONTEXT.md`, and `.codex/guidelines/*.md`
...
```markdown
See `.codex/CONTEXT.md` for the current state of work (updated by `$codex-checkpoint`).
See `.codex/guidelines/architecture.md` for global boundaries.
See `.codex/guidelines/db.md` for database patterns.
```

NEVER add `.codex/JOURNAL.md` as a loaded context reference. JOURNAL is intentionally excluded from session context to save tokens. If you find such an import or auto-load instruction in `AGENTS.md`, `.codex/CODEX.md`, or `.codex/guidelines/`, remove it and warn the user in your final summary.
...
- Remove stale generated-marker comments from CLAUDART base templates.
...
- Confirm `AGENTS.md` and `.codex/CODEX.md` reference `.codex/CONTEXT.md`.

For `.codex/JOURNAL.md`:

- Confirm it exists. If not, create it from the CLAUDART template.
- Search `AGENTS.md`, `.codex/CODEX.md`, and `.codex/guidelines/` for instructions that auto-load `.codex/JOURNAL.md`. If found, remove them and warn the user.
