...
- Use `/spec` instead when the requested outcome is a defined mission needing POC-frozen intent, phases, standing approval, and multi-session convergence. When the project or product itself needs documentation before an implementation mission, use `/project-docs` if installed; otherwise clarify intent within the current work and follow the repository's documentation convention.
