...
Tell the user, briefly: the baton is written, what was routed to knowledge or the task file, and that the next `/start` will pick the baton up. Do not continue working after a handoff; the session is considered closed.

## Consumption Contract (what the next session does)

`/start` owns consumption — the flow lives there. The contract this file must honor:

- the next `/start` reads the baton in full, surfaces it, and offers to resume;
