...
- **Post-normalization path traversal.** Validator runs _before_ a Unicode/encoding transform, then the transform produces `.` or `/` from non-ASCII codepoints (e.g. `Buffer.from(name, 'latin1').toString('utf8')` turns `U+012E`→`.` and `U+012F`→`/`); NFKC/NFKD collapses fullwidth `／`→`/`. The assertion `path.resolve(base, name).startsWith(base + path.sep)` must run on the _final_ name actually written.
- **JWT default/fallback secrets.** `process.env.JWT_SECRET || 'changeme'`, env-variable renames where the new name's default is the same insecure literal, and any path that _decodes_ (not verifies) a JWT and trusts the `sub` (logout, refresh, unsubscribe).
- **Embed/iframe whitelists keyed on `Origin`/`Referer`/`parentOrigin`** as the sole gate — these are client-controlled and spoofable.
- **Stale-authorization caches.** JWT/session validation that trusts a Redis/LRU-cached user row for `is_active`, `deleted_at`, `role`, `is_super_admin`. After disable/demote/delete or password change, the stale cache extends the attacker's window until TTL. Either bypass cache for auth-decisive fields, or event-invalidate _before_ the auth check.
...
- **Bootstrap / seed scripts auto-creating a super-admin** with predictable creds on startup (Docker `entrypoint.sh`, Helm post-install, `seed.ts`, `OnApplicationBootstrap`). Also flag seeders that _update_ an existing account into super-admin based on an env var (privilege escalation by env).
...
- **Account pre-hijacking.** Registration reserves the email _and_ stores the registrant's password _before_ email ownership is proven; later verification only checks `email + OTP` and flips `email_verified_at` on the attacker-created row — login then works with the attacker's password. Either bind verification to the original provisional session, require password re-entry on verify, or don't create the row until verification succeeds.
- **Rate-limit dimension bypass.** Limits keyed on an attacker-mintable identifier (`visitorId` from session-exchange, anonymous JWT `sub`, `deviceId`, fresh OAuth state) without _also_ keying on a stable dimension (IP, owner, target resource) lets one client multiply the limit by minting N identifiers.
...
- **WebSocket & SSE origin validation.** Browsers do _not_ enforce same-origin on WS upgrade — server must validate `Origin` against an allowlist. For SSE, `new EventSource(url)` must be allowlisted; never `eval` message data.
...
- **Web cache deception.** `/account/profile.css` or `/api/user.json` cached as static by extension despite returning user-specific content. Server must set `Cache-Control: private, no-store` for authenticated responses _and_ the edge must honor it.
...
**Rule:** never echo a discovered secret in full. Mask all but the leading 4 and trailing 4 characters (`AKIA****WXYZ`). Report file:line and the secret _type_ only.
...
**Write the report to a file — do not dump it into the chat.** Save the full report as a Markdown file at the project root named `security-audit-<YYYY-MM-DD>.md` (today's UTC date). If a file with that name already exists, append a zero-padded suffix — `security-audit-<YYYY-MM-DD>-002.md`, `-003`, and so on — so you never overwrite a prior report. This single report file is the only thing you are permitted to write.

After writing the file, print to chat **only**: the report file path, the Summary table, and the 2–4 sentence executive summary. The file is the deliverable; the chat gets the headline.

The file's contents must use this exact structure, in Markdown. Nothing else.
...
- **Read-only on the codebase.** Never use Edit. The only file you may Write is the single audit-report file from Phase 4 — never create, modify, or delete source, config, or any other file. Never run code, tests, or installers. Never call external services. Never read `.env` / `.env.*` files; if you must check whether one exists or is gitignored, use `git ls-files` and `ls`, not their contents.
...
- **One report per run.** Write exactly one report file and print only the headline summary described in Phase 4. Do not append chatter.
