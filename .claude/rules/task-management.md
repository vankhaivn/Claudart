...
- **`done`**: completed. Move file to `tasks/done/`. Append one line to `.claude/JOURNAL.md`.
...
- **Do NOT modify any code file.** No `Write`, no `Edit`, no `NotebookEdit` against anything outside `.claude/tasks/` (or `.codex/tasks/` for the Codex mirror).
- **Allowed**: read-only exploration (`Read`, `Grep`, `Glob`, `git log/diff/status`), and creating or editing the task file itself (and `index.md`).
...
3. Bump `updated:`.
4. Move the file to `.claude/tasks/done/`.
5. Append one line to `.claude/JOURNAL.md`:
   ```
   YYYY-MM-DD | completed | <slug> — <one-line outcome>, see tasks/done/<filename>
   ```
6. Update `.claude/tasks/index.md`: remove from Active, add to Recently Done.
7. If a recurring pattern emerged, propose `/learn` to graduate it into a rule.
...
- ❌ Copying a task's body (Steps / Decisions / Surprises / Memory Hints) into `.claude/CONTEXT.md`. CONTEXT may *reference* the active task by slug + path, but must never duplicate its content.
