...
Start a Codex session with a lightweight CLAUDART orientation. This skill is read-only, with one exception: consuming the handoff baton (Case H) deletes `.claudart/HANDOFF.md` once the user resumes or discards it.
...
2. Read `.claudart/CONTEXT.md` if it exists. If missing, say the project has no shared project context yet and suggest `$codex-checkpoint` after meaningful work.
