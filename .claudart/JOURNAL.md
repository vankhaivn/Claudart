...
> **DO NOT add `@.claudart/JOURNAL.md` to `.claude/CLAUDE.md`.** This file is intentionally excluded from session context to save tokens. Claude reads it only on explicit user request (e.g., "why did we choose X last month?") or during `/learn` to detect recurring patterns that should be promoted to rules.
